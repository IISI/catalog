...
public class FunctionLogDao extends AbstractGenericDao<FunctionLog, Long> implements IFunctionLogDao {
    ...
    @Override
    public List<Map<String, Object>> find4300MISReportData(Long jcScrId,
            Date startDate, Date endDate, Set<String> functions) {
        StringBuilder sql = new StringBuilder();
        sql.append("SELECT FL.FUNCTION_ID functionId, S.SCR_NO scrNo, S.PROCESS_TIME processDate ");
        sql.append("FROM JC_FUNCTION_LOG FL ");
        sql.append("LEFT JOIN JC_SCR S ON FL.JC_SCR_ID = S.ID ");
        sql.append("WHERE ");
        if (jcScrId != null) {
            sql.append("FL.JC_SCR_ID = :scrId AND ");
        }
        ...
        sql.append(" ORDER BY functionId ASC, scrNo ASC");
        logger.info("find sql = {}", sql.toString());
        Map<String, Object> params = new HashMap<String, Object>();
        params.put("scrId", jcScrId);
        params.put("startDate", startDate);
    ...
    @Override
    public List<Map<String, Object>> find4300SCRReportData(Long jcScrId,
            Date startDate, Date endDate, Set<String> functions) {
        StringBuilder sql = new StringBuilder();
        sql.append("SELECT S.SCR_NO scrNo, FL.FUNCTION_ID functionId, A.APP_ID app, S.CREATE_TIME createDate, S.PROCESS_TIME processDate, P.NAME programmer, C.NAME coordinator, S.DESCRIPTION description ");
        sql.append("FROM JC_FUNCTION_LOG FL ");
        sql.append("    LEFT JOIN JC_SCR S ");
        sql.append("        LEFT JOIN JC_APP A ON S.JC_APP_ID = A.ID ");
        sql.append("        LEFT JOIN JC_PROGRAMMER P ON S.JC_PROGRAMMER_ID = P.ID ");
        sql.append("        LEFT JOIN JC_COORDINATOR C ON S.JC_COORDINATOR_ID = C.ID ");
        sql.append("    ON FL.JC_SCR_ID = S.ID ");
        sql.append("WHERE ");
        if (jcScrId != null) {
            sql.append("FL.JC_SCR_ID = :scrId AND ");
        ...
        sql.append(" ORDER BY scrNo ASC, functionId ASC");
        logger.info("find sql = {}", sql.toString());
        Map<String, Object> params = new HashMap<String, Object>();
        params.put("scrId", jcScrId);
        params.put("startDate", startDate);
